class UserStocksController < ApplicationController
  ...
  def destroy
    @user_stock = Stock.find(params[:id]).user_stocks.where(stock_id:params[:id],user_id:current_user.id).first
    @user_stock.destroy
    respond_to do |format|
      format.html { redirect_to my_portfolio_path, notice: 'Stock was Successfully removed from my_portfolio' }
